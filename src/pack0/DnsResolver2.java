...
public class DnsResolver2 {
	...
	public static void main(String[] args) {
		try {
			new DnsResolver2(args[0]);
		} catch (IOException e) {
			e.printStackTrace();
		}
	}

	public DnsResolver2(String port) throws IOException {
	...
	private String askServer(DatagramPacket receivePacket, Cache server)
		...
		if (answer) {
			if (serverToAsk != null) {
				System.out
						.println("Answer was found in the latest data packet.");
				return serverToAsk.getIpAddress();
			} else {
				System.out
						.println("An error occured and the recieved packet had"
								+ " no information about the next server");
				return "";
			}
		} else {
			System.out
	...
	private boolean decodeMessage(DatagramPacket data) {
		...
		System.out
				.println("Information about the packet (all values in hex): \n"
						+ "Transation ID: " + transId + "\nFlags ID: "
						+ flagsId + "\nQuestion RRS: " + questRRS
						+ "\nAnswer RRS: " + answerRRs + "\nAuthority RRS: "
						+ authRRS + "\nAdditional RRS: " + additRRS);

		if (answerRRs.equals("0000") && authRRS.equals("0000")
				&& additRRS.equals("0000")) {
			serverToAsk = null;
			return true;
		}
		...
		if ((int) Long.parseLong(answerRRs, 16) > 0) {
			...
			if ((int) Long.parseLong(authRRS, 16) != 0) {
				...
				index -= 2;
				int add = (int) Long.parseLong(additRRS, 16);
				if (add != 0) {
					while (true) {
						String test = c[index] + "" + c[index + 1] + ""
								+ c[index + 2] + "" + c[index + 3] + ""
								+ c[index + 4] + "" + c[index + 5] + ""
								+ c[index + 6] + "" + c[index + 7] + ""
								+ c[index + 8] + "" + c[index + 9] + ""
								+ c[index + 10] + "" + c[index + 11];
						if (test.startsWith("c0") && test.endsWith("00010001")) {
							break;
						} else {
							index++;
						}
